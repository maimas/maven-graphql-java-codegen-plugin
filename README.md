...
- Build fails with validation errors:
  - By default, the plugin validates generated code and fails the build if issues are found. You can temporarily disable this by setting <failOnValidationError>false</failOnValidationError> for a specific server while migrating. Prefer fixing schema/template issues.
