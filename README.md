...
- [Usage](#usage)
  ...
  - [How to use generated API](#how-to-use-generated-api)
    - [Mutation - create user](#mutation---create-user)
    - [Query - get user by id](#query---get-user-by-id)
    - [Subscription - user updates](#subscription---user-updates)
    - [Complex types and nested objects](#complex-types-and-nested-objects)
...
public class Examples {
    public void subscribeToUserUpdates() {
        // Build a subscription string
        GQLQuery subscription = new Subscription().userUpdated(input ->
                input.id("123123"),
                output -> output
                    .id()
                    .email()
        ...
        // Use your preferred WebSocket client to send the subscription
        // For example using Spring's WebSocketStompClient or Reactor Netty
        // send the subscription.toString() as the GraphQL payload
    }
}
````

#### Complex types and nested objects
````
import static com.maimas.generated.UserServiceGraphQLProvider.*;

public class Examples {
    public void queryWithNestedFields() {
        GQLQuery query = new Query().findById(input ->
                input.id("123123"),
                output -> output
                    .id()
                    ...
                    .address(addr -> addr
                        .street()
                        .city(c -> c.name().country(cty -> cty.code().name()))
                    )
                    .roles(r -> r.name().permissions(p -> p.code().description()))
        );
    }
}
````
...
public class Examples {
    public void queryWithArguments() {
        GQLQuery query = new Query().findByEmail(input ->
                input.email("john@example.com").includeInactive(true),
                output -> output.id().email().status());

        // The argument values are provided via the input builder above.
        // Post query.toString() to your GraphQL endpoint using your HTTP client.
    }
}
````
...
public class UserClient {
  ...
  public UserClient(RestTemplate restTemplate) { this.restTemplate = restTemplate; }

  public String createUser(String email, String password) {
    var userInput = new com.maimas.generated.UserServiceGraphQLProvider.Types.UserInput();
    userInput.email = email; userInput.password = password;

    GQLQuery query = new Mutation().create(in -> in.user(userInput), out -> out.id().email());

    return restTemplate.postForObject("http://localhost:8080/graphql", query.toString(), String.class);
  }
}
````
...
### Warning levels and ignoring rules
- The validator now emits different severities: ERROR (fails validation), WARNING, INFO.
- Additional advisory check: balanced parentheses is reported as WARNING and does not fail generation.
- You can ignore specific validation rules by ID using UserConfig.setIgnoredValidationRules or Maven config:
  - Supported IDs: BASIC_SYNTAX, BRACES_BALANCED, SEMICOLON_TERMINATION, PARENTHESES_BALANCED
  - Example (Maven):
```
<ignoredValidationRules>
  <ignoredValidationRules>PARENTHESES_BALANCED</ignoredValidationRules>
</ignoredValidationRules>
```
